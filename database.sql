...
INSERT INTO Products	(ProductID, Model, Category, Price, Description, Manufacturer, Color, 
	...
	VALUES
(1, 'GA-B459CLWL', 2, 37999, 'Холодильник LG GA-B459CLWL в матовом серебристом корпусе имеет 
	солидную вместительность – 341 л. Благодаря этому в нем можно разместить большие продуктовые 
	запасы. Морозильная камера на 107 л состоит из 3 выдвижных ящиков с возможностью замораживания 
	до 16 кг продуктов в сутки. Режим «Отпуск» позволит минимизировать и без того символическое 
	энергопотребление во время вашего отсутствия. Индикатор температуры и внешний дисплей делают 
	модель понятной в обращении. Блокировка управления отвечает за безопасность эксплуатации.',
	2, 2, 59.5, 186, 68.2, 12, 309, 2, 'Data/Images/Products/Refregerators/GA-B459CLWL.jpg',
	2, 1, 1, 234, 107, 1),
(2, 'SBS510M', 2, 49999, 'Холодильник DEXP SBS510M, оборудованный распашными дверями, обладает 
	значительным полезным объемом: он равен 510 л. На холодильную и морозильную камеры приходится 
	335 и 175 л соответственно. Модель подойдет для размещения значительного количества продуктов и 
	напитков. Холодильник оснащен дисплеем, на котором отображается информация о температуре в обеих 
	камерах. В число функций устройства входят суперохлаждение, суперзаморозка, режим отпуска и 
	блокировка от детей. Заниматься размораживанием холодильника не потребуется: в обеих камерах 
	размораживание происходит автоматически, с использованием технологии No Frost.', 4, 2, 
	89.5, 178.8, 71, 36, 405, 2, 'Data/Images/Products/Refregerators/SBS510M.png', 3, 0,
	0, 335, 175, 1);
GO
...
INSERT INTO Products	(ProductID, Model, Category, Price, Description, Manufacturer, Color, 
	...
	VALUES
(3, 'Mi TV 4S 50', 1, 38999, 'Телевизор LED Xiaomi Mi TV 4S 50 является воплощением элегантного 
фирменного стиля и технологичности. Он оборудован 50-дюймовым экраном с разрешением 3840x2160 
пикселей, технологией расширенного динамического диапазона цветов и подсветкой Direct LED. 
Это означает, что с любого угла обзора картинка будет впечатлять детализацией и естественной 
цветопередачей. Тонкая рамка позволяет максимально погрузиться в просмотр. Акустическая система 
выходной мощностью 20 Вт воспроизводит чистый звук с пространственным эффектом.', 3, 3, 12, 
'Data/Images/Products/TVs/MiTV4S50.jpg', 1, 7, 2, 1, 1, 1, 1),
(4, 'UE43TU7002UXRU', 1, 37499, '43-дюймовый телевизор LED Samsung UE43TU7002UXRU сочетает в 
себе высокий уровень функциональных возможностей и безупречное качество изображения. Разрешение 
модели (3840x2160) соответствует формату 4K. В устройстве реализована поддержка HDR. Подсветка 
экрана Edge LED отличается высоким уровнем равномерности.', 1, 1, 12, 
'Data/Images/Products/TVs/UE43TU7002UXRU.jpg', 2, 5, 2, 1, 2, 1, 1),
(5, '65UP75006LF', 1, 54999, 'Телевизор LED LG 65UP75006LF является удобным аппаратом для просмотра 
как стандартных телевизионных программ, так и для потоковой трансляции видео из Интернета. 
Он выполнен в черном корпусе с диагональю экрана 65" (165 см) и устанавливается на подставку, 
которая крепится по обе стороны корпуса, обеспечивая устойчивость устройства и не позволяя ему наклоняться. 
Благодаря модулю Wi-Fi, установленному на LG 65UP75006LF, вы сможете насладиться плавностью потоковой 
трансляции видео без задержек.', 2, 1, 12, 'Data/Images/Products/TVs/65UP75006LF.jpg', 1, 10, 2, 1, 3, 1, 1);
GO